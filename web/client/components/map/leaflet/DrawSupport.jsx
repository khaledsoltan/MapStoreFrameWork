...
// Add a helper for pointToLayer with icon support
function pointToLayerWithIcon(feature, latLng, style) {
    const iconUrl = feature?.style?.iconUrl || feature?.properties?.iconUrl;
    if (iconUrl) {
        return L.marker(latLng, {
            icon: L.icon({
                iconUrl,
                iconSize: feature?.style?.iconSize || [25, 41],
                iconAnchor: feature?.style?.iconAnchor || [12, 41]
            })
        });
    }
    // fallback to circle marker
    return L.circleMarker(latLng, style || { radius: 8, color: '#ff0000' });
}
...
class DrawSupport extends React.Component {
    ...
    */
    ...
    addLayer = (newProps) => {
        ...
        const vector = L.geoJson(null, {
            pointToLayer: function(feature, latLng) {
                return pointToLayerWithIcon(feature, latLng, newProps.style);
            },
    ...
    addGeojsonLayer = ({features, projection, style}) => {
        this.clean();
        let geoJsonLayerGroup = L.geoJson(features, {
            style: (f) => f.style || style,
            pointToLayer: (f, latLng) => {
                let center = reproject({x: latLng.lng, y: latLng.lat}, projection, "EPSG:4326");
                return pointToLayerWithIcon(f, L.latLng(center.y, center.x), style);
            }
        });
    ...
    replaceFeatures = (newProps) => {
        if (!this.drawLayer) {
            ...
            this.drawLayer.options.pointToLayer = (f, latLng) => {
                ...
                return pointToLayerWithIcon(f, L.latLng(center.y, center.x), newProps.style);
            };
